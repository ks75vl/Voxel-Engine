	...
	void ThreadWorker() {
		...
		while (!this.isStopped) {
			...
			if (File.Exists(this.seed_number.ToString())) {
				...
				binaryReader.BaseStream.Seek(binaryReader.BaseStream.Length - 4 * 4, SeekOrigin.Begin);
			...
			short max = 0;
			short min = System.Int16.MaxValue;
			short temp = 0;
				...
				for (int z = 0; z < wz; z++) {
					...
					for (int i = 0; i < 16; i++) {
						...
						for (int j = 0; j < 16; j++) {
							...
							temp = (short)((Noise.Sum(point, 5f, 6, 2f, 0.5f).value + 0.6f) * 128 + 250 * 16);
			...
			binaryWriter.Write(wx);
			binaryWriter.Write(wz);
			binaryWriter.Write(this.seed_number);
